...
```{r}
model13 <- clmm(Alfalfa_Maturity ~ selected_type * LOC + (1|LOC:Rep) + (1|Entry), data = data)
summary(model13)
aov_clmm_talbe <- Anova.clmm(model13, type = "II")
...
```{r}
model14 <- clmm(Alfalfa_Regrowth ~ selection_cat * LOC + (1|Entry) + (1|LOC:Rep), data = data)
summary(model14)
aov_clmm_table <- Anova.clmm(model14, type = "III")
...
model15 <- lmer(First_Yr_Regrowth_Vigor ~ selected_type * LOC + (1|LOC:Rep) + (1|Entry), data = data)
summary(model14)
aov_clmm_table <- Anova.clmm(model14, type = "III")
...
model16 <- lmer(First_Summer_ALF_Biomass_sqrt ~ sel_cat2*LOC + (1|Entry) + (1|LOC:Rep), data=data)
aov_select_table <- anova(model16)
table = flextable(data = aov_select_table %>%
                    rownames_to_column(" ")) 
bold(table, bold = TRUE, part = "header")
