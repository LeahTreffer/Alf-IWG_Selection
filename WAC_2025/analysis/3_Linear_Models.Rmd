...
emmeans(model5, pairwise ~ selected_cat)
```

tests if selection effects are consistent or location-dependent

```{r}
model6 <- lmer(First_Summer_ALF_Biomass_sqrt ~ selected_cat * LOC + (1|LOC:Rep) + (1|Entry), data = data2)
anova(model6)
emmeans(model6, pairwise ~ selected_cat | LOC)
```
...
```{r}
model7 <- lmer(First_Yr_Regrowth_Vigor ~ selected_cat * LOC + (1|LOC:Rep) + (1|Entry), data = data2)
anova(model7)
emmeans(model7, pairwise ~ selected_cat | LOC)
```
...
```{r}
model8 <- lmer(First_Summer_Maturity ~ selected_cat * LOC + (1|LOC:Rep) + (1|Entry), data = data2)
anova(model8)
emmeans(model8, pairwise ~ selected_cat | LOC)
```

Summer Height

```{r}
model9 <- lmer(First_Summer_Alfalfa_Height ~ selected_cat * LOC + (1|LOC:Rep) + (1|Entry), data = data2)
anova(model9)
